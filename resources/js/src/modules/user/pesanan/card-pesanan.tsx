...
export default function CardPesanan({ data }: { data: IOngkir }) {
	...
	const handleSetTelahSampai = () => {
		...
		changeStatus.mutate(toSave, {
			...
			onError: (e: any) =>
				toastError(
					e?.response?.data?.message ?? "Gagal mengubah pesanan"
				),
		});
	};
	...
	return (
		<div
			key={ data.id }
			className="rounded-md overflow-hidden border border-black/10 shadow-xs"
		>
			{ match([
				data.is_confirmed_by_admin,
				data.resi,
				data.telah_sampai,
				data.pesanan?.[0].status,
			])
				...
				.with([1, P.string, 1, "dibayar"], () => (
					<div className="w-full p-5 bg-blue-400 text-white flex items-center gap-5">
						<GoHomeFill size={ 20 } />
						<span>
							Pesanan telah sampai, terimakasih telah berbelanja.
						</span>
					</div>
				))
				.otherwise(() => null) }
				...
				<div className="flex justify-between items-start flex-col sm:flex-row">
					...
					<div className="flex gap-2">
						<button
							onClick={ () => setShow((v) => !v) }
							className={ twMerge(
								show ? "bg-orange-500" : "bg-sky-500",
								"text-xs text-white px-4 py-2 rounded-md h-fit mt-2 ml-auto sm:ml-0 sm:mt-0"
							) }
						>
							{ show ? "Close" : "Detail" }
						</button>
						{ match([
							...
							.with([null, 1, "belum-bayar", 0], () => (
								<ModalMetodeBayar
									pesananGrup={ data.pesanan_grup }
								/>
							))
				...
				{ match([
					...
					.with([null, 1, "belum-bayar", 0], () => (
						<p className="text-black text-sm mt-3">
							Sudah melakukan pembayaran? Cek status pembayaran <a target="_blank" href={ `/payment-status?pesanan_grup=${data.pesanan_grup}` } className="underline text-blue-800">di sini.</a>
						</p>
					))
					.otherwise(() => null) }
				{ show && (
