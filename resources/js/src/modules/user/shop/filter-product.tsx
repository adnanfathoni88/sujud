...
function FilterDropdown({
    ...
    const [selected, setSelected] = useState<number | null>(
        isNaN(search?.kategori_id) ? null : Number(search?.kategori_id)
    );
                        ...
                        {categories.map((c) => (
                            <MenuItem key={c.id}>
                                <button
                                    ...
                                    className={twMerge(
                                        "block rounded-lg py-2 px-3 transition hover:bg-black/5 flex items-center w-full",
                                        c.id == selected && "bg-black/5"
                                    )}
                                >
                                    {c.id == selected && (
                                        <FaCheck size={16} className="mr-2" />
                                    )}
                                    <p className="font-semibold text-black/90 m-0">
                                        {c.nama}
                                    </p>
                                </button>
                            </MenuItem>
