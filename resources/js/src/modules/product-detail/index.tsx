				...
				<div className="flex items-center justify-between mb-6 flex-col sm:flex-row">
					...
					{ Boolean(data?.response) && (
						<div className="flex gap-3 items-center">
							...
							<button className="hover:text-primary" onClick={ handleDelete(data?.response?.id) }>
								<CiTrash size={ 20 } />
							</button>
						</div>
					) }
						...
						.with(true, () => (
							<dl className="">
								<div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
									...
									<dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{ data.response.nama }</dd>
								</div>
								<div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
									<dt className="text-sm font-medium leading-6 text-gray-900">Kode Produk</dt>
									<dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{ data.response.kode }</dd>
								</div>
								<div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
									<dt className="text-sm font-medium leading-6 text-gray-900">Kategori</dt>
									<dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{ data.response.kategori.nama }</dd>
								</div>
								<div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
									<dt className="text-sm font-medium leading-6 text-gray-900">Deskripsi</dt>
									<dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{ data.response.deskripsi }</dd>
								</div>
							</dl>
						))
						.otherwise(() => <Loader />) }
