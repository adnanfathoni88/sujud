...
const CategoryListModule: React.FC = () => {
	...
	const handleDelete = (id: number) => () => deleteCategory.mutate({ id }, {
			...
			<div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
				...
				<div className="flex flex-col">
					...
					{ match(Array.isArray(data?.response))
						.with(true, () => data?.response.map((c, key) => (
							<div
								className={ twMerge("grid grid-cols-3 sm:grid-cols-5", key !== (data.response.length - 1) && 'border-b border-stroke dark:border-strokedark') }
								key={ key }
							>
