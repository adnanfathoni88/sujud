						...
						<table className="min-w-[100%]">
							<thead>
								<tr>
									...
									<th className="p-4 whitespace-nowrap text-left">Tanggal</th>
									<th className="p-4 whitespace-nowrap text-left">User</th>
									<th className="p-4 whitespace-nowrap text-left">Konten</th>
									<th className="p-4 whitespace-nowrap text-left">Rating</th>
									<th className="p-4 whitespace-nowrap text-left">Aksi</th>
								</tr>
							</thead>
							<tbody>
								{ match([Boolean(data), Array.isArray(data?.response?.data)])
									.with([true, true], () => data?.response.data.map((c, key) => (
										<tr key={ c.id } className="align-top">
											...
											<td className="py-3 px-4 whitespace-nowrap">{ convertISODateToCustomFormat(c.created_at) }</td>
											<td className="py-3 px-4 whitespace-nowrap">{ c.user_id }</td>
