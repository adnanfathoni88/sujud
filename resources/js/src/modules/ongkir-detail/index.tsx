...
export default function OngkirListModule() {
    ...
    const hasOutOfStock = useMemo(() => {
        ...
        for (let i = 0; i < data.response.pesanan.length; i++) {
            outOfStock =
                data.response.pesanan[i].varian.stok <
                data.response.pesanan[i].qty;
            if (outOfStock) break;
        }
    ...
    return (
        <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-5">
            <div className="flex items-center justify-between mb-6 flex-col sm:flex-row">
                <h4 className="text-xl font-semibold text-black dark:text-white me-auto sm:me-0 capitalize">
                    Daftar Pesanan {data?.response?.pelanggan?.nama}
                </h4>
            </div>
            <div className="mt-6">
                <dl className="">
            ...
            <div className="overflow-x-auto mt-6">
                <div className="align-middle">
                    <table className="min-w-[100%]">
                        <thead>
                            <tr>
                                <th className="border bg-zinc-200 dark:bg-black/50 p-4 whitespace-nowrap text-left">
                                    No.
                                </th>
                                <th className="border bg-zinc-200 dark:bg-black/50 p-4 whitespace-nowrap text-left">
                                    Produk
                        ...
                        <tbody>
                            {Array.isArray(data?.response?.pesanan) &&
                                data?.response?.pesanan.map((c, key) => (
                                    <tr key={c.id} className="align-top">
                                        <td className="border py-3 px-4 whitespace-nowrap">
                                            {key + 1}
                                        </td>
                                        <td className="border py-3 px-4 whitespace-nowrap">
                                            {c?.varian?.produk?.nama}
                                        </td>
                                        <td className="border py-3 px-4 whitespace-nowrap">
                                            {c?.varian?.ukuran}
                                        </td>
                                        ...
                                        <td
                                            className={twMerge(
                                                "border border-black py-3 px-4 whitespace-nowrap",
                                                c?.qty > c?.varian?.stok &&
                                        ...
                                        <td className="border py-3 px-4 whitespace-nowrap">
                                            Rp. {c?.total?.toLocaleString()}
                                        </td>
                                        <td className="border py-3 px-4 whitespace-nowrap">
                                            Rp. {c?.diskon?.toLocaleString()}
                                        </td>
                                    </tr>
                                ))}
                            {Array.isArray(data?.response?.pesanan) && (
                                <tr>
                                    <td
                                        colSpan={7}
                                        className="border py-3 px-4 whitespace-nowrap"
                                    >
                                        Total
                                    </td>
                                    <td
            ...
            {Boolean(data?.response) &&
                !data?.response?.is_confirmed_by_admin && (
                    <KonfirmasiPesanan
